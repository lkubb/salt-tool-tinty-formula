...
We will see a quick setup to configure the ``tool_tinty`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_tinty`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_tinty`` formulas under ``/srv/salt/tool_tinty/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_tinty/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_tinty/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_tinty/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_tinty/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_tinty`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_tinty`` formulas under ``/srv/salt/tool_tinty/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_tinty/parameters/roles
...
We create a configuration for the role ``tool_tinty/server`` in ``/srv/salt/tool_tinty/parameters/roles/tool_tinty/server.yaml``:
...
We create another configuration for the role ``tool_tinty/client`` in ``/srv/salt/tool_tinty/parameters/roles/tool_tinty/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_tinty/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_tinty/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_tinty/parameters/dns:domain/  '/srv/salt/tool_tinty/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-tinty-as-json:
      test.nop:
        - name: {{ tinty | json }}
...
.. code-block:: sls
    ...
    tinty-config-file-file-managed:
      file.managed:
        - name: {{ tinty.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='tinty-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ tinty.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``tinty`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ tinty['winner'] }}
